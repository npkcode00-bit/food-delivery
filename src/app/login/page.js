...
function LoginForm() {
  ...
  async function handleFormSubmit(ev) {
    ...
    try {
      ...
      if (res?.error) setError(getErrorMessage(res.error));
      else router.push('/');
    } catch {
  ...
  return (
    <section className="relative">
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12 py-10">
        <div className="mx-auto w-full max-w-md rounded-2xl border border-white/30 bg-white/70 p-6 shadow-sm backdrop-blur-xl">
          <h1 className="text-center text-2xl font-semibold text-zinc-900">Welcome back</h1>
          <p className="mt-1 text-center text-sm text-zinc-600">Sign in to continue</p>
